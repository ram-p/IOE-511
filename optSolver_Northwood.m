...
while (k < options.max_iterations) && (norm_g > options.term_tol*max(1, norm_g_init))    % Termination condition
    ...
    switch method.name
        ...
        case 'DFP'
            ...
            Hest_old = Hest; Hest = Hest_new;
        case 'TRNewtonCG'
            [x_new, f_new, g_new, H_new, delta] = TRNewtonCGStep(x, f, g, H, problem, method, options);
            % Update Hessian now.
            H_old = H; H = H_new;
        case 'TRSR1CG'
            [x_new, f_new, g_new, Hest_new, delta] = TRSR1CGStep(x, f, g, Hest, problem, method, options);
